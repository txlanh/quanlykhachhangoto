    ...
    <main className="min-h-screen bg-gray-50 pb-32">
      <div className="bg-white px-4 py-4 sticky top-0 z-20 border-b border-gray-100 shadow-sm flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${customer.customer_type === 'cash' ? 'bg-emerald-100 text-emerald-700' : 'bg-purple-100 text-purple-700'}`}>
      ...
      <div className="bg-white px-6 py-6 pb-8 border-b-2 border-gray-100 rounded-b-3xl shadow-sm relative z-10 mb-6">
        ...
        <div className="space-y-4 mt-6">
          ...
          {/* Bắt đầu phần hiển thị Giá và Phụ kiện */}
          <div className="grid grid-cols-2 gap-3 mt-4">
            <div className="bg-blue-50/50 p-3 rounded-xl border border-blue-100">
              <p className="text-xs text-blue-600/70 font-semibold mb-1 uppercase tracking-wider">Giá chốt lăn bánh</p>
              <p className="font-bold text-blue-900 text-lg">
                {customer.gia_chot_lan_banh ? customer.gia_chot_lan_banh.toLocaleString('vi-VN') : '0'} đ
              </p>
            </div>
            <div className="bg-blue-50/50 p-3 rounded-xl border border-blue-100">
              <p className="text-xs text-blue-600/70 font-semibold mb-1 uppercase tracking-wider">Giá hóa đơn</p>
              <p className="font-bold text-blue-900 text-lg">
                {customer.gia_hoa_don ? customer.gia_hoa_don.toLocaleString('vi-VN') : '0'} đ
              </p>
            </div>
          </div>

          {customer.phu_kien_tang && (
            <div className="bg-orange-50/50 p-4 rounded-xl mt-3 border border-orange-100">
              <p className="text-xs text-orange-600/70 font-semibold mb-2 uppercase tracking-wider flex items-center gap-1.5">
                <span className="w-1.5 h-1.5 rounded-full bg-orange-500"></span>
                Phụ kiện tặng kèm
              </p>
              <p className="font-medium text-orange-900 text-[15px] whitespace-pre-wrap leading-relaxed">
                {customer.phu_kien_tang}
              </p>
            </div>
          )}
          {/* Kết thúc phần hiển thị Giá và Phụ kiện */}
